...
function StatusPill({status}: {status: string}) {
  return <span className={`status-pill ${status}`}>{status === "stable" ? "1.0 draft" : status}</span>;
}
...
function Home({go}: {go: (path: string) => void}) {
  ...
  return <article className="home">
    ...
    <section className="release-band"><div><i/><span>Documentation target</span><b>BV Node Pack 1.0.0</b></div><div><span>Reference coverage</span><b>{publicNodes.length} public nodes</b></div><div><span>Assets awaiting production</span><b>{assets.filter(asset => asset.status !== "approved").length}</b></div></section>
  </article>;
}
...
function NodePage({node}: {node: NodeContract}) {
  ...
  const statusText = node.status === "deprecated" ? "Deprecated compatibility node. Prefer the replacement described below." : node.status === "experimental" ? "Experimental: behavior and results may change between releases." : "Targeted for BV Node Pack 1.0.0; documentation remains under review until release verification.";
