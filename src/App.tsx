...
function Home({go}: {go: (path: string) => void}) {
  ...
  return <article className="home">
    ...
    <section className="release-band"><div><i/><span>Documentation target</span><b>BV Node Pack 1.2.1</b></div><div><span>Reference coverage</span><b>{publicNodes.length} public nodes</b></div><div><span>Assets awaiting production</span><b>{assets.filter(asset => asset.status !== "approved").length}</b></div></section>
  </article>;
}
...
function NodePage({node}: {node: NodeContract}) {
  ...
  const statusText = node.status === "deprecated" ? "Deprecated compatibility node. Prefer the replacement described below." : node.status === "experimental" ? "Experimental: behavior and results may change between releases." : "Documented for BV Node Pack 1.2.1; runtime and visual evidence remains scoped to the verification stated on each page.";
